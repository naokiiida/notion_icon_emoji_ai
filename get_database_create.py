...
def get_database_pages(database_id, notion_token):
	...
	if response.status_code == 200:
		...
		print(steps['trigger']['event']['last_edited_time'])
		last_edit = datetime.fromisoformat(steps['trigger']['event']['last_edited_time'][:-1]).replace(tzinfo=timezone.utc)
		created = datetime.fromisoformat(steps['trigger']['event']['last_edited_time'][:-1]).replace(tzinfo=timezone.utc)
		print(last_edit)
		print(created)
